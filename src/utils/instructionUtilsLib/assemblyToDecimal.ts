 ...
 * @param type
 */
function assemblyToDecimal(type : IRtype | IItype | IJtype) : DecimalSchema[] | undefined {

    // Turns an entry array of 2 elements into a valid object mathcing DecimalSchema
    const objectify = (entry :any[]): DecimalSchema => {
    ...
    try {
        const decimal = extractEntries(type).map(entry => {
